...
echo "=== Fixing StyleGenieAI Bedrock Integration ==="
...
# Create a more comprehensive Bedrock policy
echo "Creating comprehensive Bedrock policy document..."
...
echo "Attaching enhanced Bedrock policy to role..."
aws iam put-role-policy --role-name $ROLE_NAME --policy-name EnhancedBedrockAccess --policy-document file://bedrock-policy.json

# Create the Lambda function file with improved Bedrock integration
echo "Creating updated Lambda function with improved Bedrock integration..."
...
# Set the model ID to use an available model - now with fallbacks
...
def invoke_bedrock_with_retries(prompt, max_retries=2):
            ...
            try:
                ...
                if bedrock is None:
                    ...
                    try:
                        ...
                        response = bedrock_test.invoke_model(
                            modelId=model_id,
                            body=json.dumps({
                        ...
                        print("Bedrock test successful, proceeding with actual request")
                    except Exception as e:
    ...
    print("All Bedrock invocation attempts failed, returning None")
    return None
...
Do not include image_url fields in your response. The system will generate those separately."""
        ...
        # Invoke Bedrock with retries and fallbacks
        ...
        if ai_response:
            ...
            print("No response from Bedrock, returning default response")
            return format_response(200, get_default_response(occasion, body_type))
...
echo "=== Bedrock Integration Fix Completed ==="
echo "The Lambda function has been updated with improved Bedrock integration:"
echo "- Added comprehensive Bedrock permissions"
echo "- Added multiple model fallbacks"
echo "- Added retry mechanism"
echo "- Improved error handling and logging"
echo ""
echo "Next steps:"
echo "1. Run ./check-lambda-logs.sh to verify if Bedrock integration is working"
echo "2. Try the AI Style Advisor page again"
